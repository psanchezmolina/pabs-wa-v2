...
(() => {
  ...
  async function fetchStatus() {
    try {
      ...
      if (!response.ok) {
        ...
        // Si es servicio no configurado, mostrar mensaje específico
  ...
  async function generatePairingCode() {
    ...
    if (!/^\d{1,15}$/.test(phoneNumber)) {
      alert('Formato inv�lido. Ingresa solo d�gitos (sin el signo +), m�ximo 15 caracteres.');
      phoneInput.focus();
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.pairingCode) {
        ...
        alert('El c�digo de vinculaci�n no est� disponible. Te cambiaremos al m�todo de QR Code.');
      ...
      pairingLoading.style.display = 'none';
      alert(`Error al generar c�digo de vinculaci�n: ${error.message}`);
      btnGeneratePairing.disabled = false;
    }
  }
